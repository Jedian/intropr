...
def maxSuppress(g, theta):    
    ...
    for x in range(1, g.shape[0]-1):
        for y in range(1, g.shape[1]-1):
            ...
            #3
            if converted_angle == 0:
                max_sup[x][y] = max(g[x][y], g[min(x+1, g.shape[0]-1)][y], g[max(x-1, 0)][y])
            elif converted_angle == 45:
                max_sup[x][y] = max(g[x][y], g[max(x-1, 0)][min(y+1, g.shape[1]-1)], g[min(x+1, g.shape[0]-1)][max(y-1, 0)])
            elif converted_angle == 90:
                max_sup[x][y] = max(g[x][y], g[x][max(y-1, 0)], g[x][min(y+1, g.shape[1]-1)])
            else:
                max_sup[x][y] = max(g[x][y], g[max(x-1, 0)][max(y-1, 0)], g[min(x+1, g.shape[0]-1)][min(y+1, g.shape[1]-1)])
